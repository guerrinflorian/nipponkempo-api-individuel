...
export default async function usersRoutes(fastify, options) {
  ...
  fastify.post("/register", async (request, reply) => {
    const { email, password, first_name, last_name, id_role, id_club } =
      request.body;
    try {
      ...
      const verificationLink = `https://nipponkempo-api-individuel.vercel.app/verify-email?token=${verificationToken}`;
  ...
  fastify.get("/verify-email", async (request, reply) => {
    ...
    try {
      ...
      reply.redirect(
        "https://nipponkempo-api-individuel.vercel.app/verification-success"
      );
    } catch (err) {
      fastify.log.error(err);
      reply.status(500).send("Erreur serveur");
  ...
  fastify.post("/resend-verification-email", async (request, reply) => {
    ...
    try {
      ...
      const { data: user, error } = await supabase
        .from("users")
        .select(
          "id, email_verified, email_verification_token, email_verification_expiry"
        )
        .eq("email", email)
        .single();
  ...
  fastify.delete(
    "/clubs/:clubId/assistant-managers/:id",
    { preValidation: verifyJWT },
    async (request, reply) => {
      const clubId = Number(request.params.clubId);
      ...
      try {
        ...
        if (selErr || !u) {
          return reply
            .code(404)
            .send({ message: "Sous-gestionnaire introuvable" });
        }
